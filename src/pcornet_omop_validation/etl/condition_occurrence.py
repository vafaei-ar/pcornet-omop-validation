...
# Historical PCORnet mappings are retained only when the referenced concept exists
# in the loaded vocabulary. Unknown/obsolete provenance and status concepts become 0.
DX_ORIGIN_TYPE_MAP = {
    "OD": 32817,
    "BI": 32821,
    "CL": 32810,
...
def _validated_existing_map(connection, schema: str, mapping: dict[str, int]) -> tuple[dict[str, int], dict[str, int]]:
...
def transform_condition_occurrence(config: EtlConfig) -> ConditionOccurrenceTransformResult:
    policies = config.raw.get("policies", {}) or {}
    if policies.get("missing_required_date") != "exclude":
        raise RuntimeError(
            "The validated condition stage currently requires policies.missing_required_date=exclude"
        )
    if policies.get("unmapped_standard_concept") != "concept_zero":
        raise RuntimeError(
            "The validated condition stage currently requires policies.unmapped_standard_concept=concept_zero"
        )
    if policies.get("condition_sources") != "include_both":
        raise RuntimeError(
            "The validated condition stage currently requires policies.condition_sources=include_both"
        )
    ...
    try:
        with engine.connect() as connection:
            ...
            diagnosis_source_rows = _scalar(
                connection, f"SELECT COUNT_BIG(*) FROM [{source_schema}].[PCORnet_DIAGNOSIS]"
            )
            condition_source_rows = _scalar(
                connection, f"SELECT COUNT_BIG(*) FROM [{source_schema}].[PCORnet_CONDITION]"
            )
            ...
            WITH classified AS (
              SELECT d.*, p.person_id,
                     CASE
                       WHEN d.DIAGNOSISID IS NULL OR LTRIM(RTRIM(CAST(d.DIAGNOSISID AS nvarchar(max)))) = '' THEN 'missing_id'
                       WHEN d.PATID IS NULL OR LTRIM(RTRIM(CAST(d.PATID AS nvarchar(max)))) = '' THEN 'missing_patid'
            ...
            diag_counts = dict(
                connection.execute(
                    text(diag_cte + " SELECT record_status, COUNT_BIG(*) FROM classified GROUP BY record_status")
                ).fetchall()
            )
            ...
            WITH classified AS (
              SELECT c.*, p.person_id,
                     CASE
                       WHEN c.CONDITIONID IS NULL OR LTRIM(RTRIM(CAST(c.CONDITIONID AS nvarchar(max)))) = '' THEN 'missing_id'
                       WHEN c.PATID IS NULL OR LTRIM(RTRIM(CAST(c.PATID AS nvarchar(max)))) = '' THEN 'missing_patid'
                       WHEN p.person_id IS NULL THEN 'unlinked_person'
                       WHEN c.ONSET_DATE IS NULL AND c.REPORT_DATE IS NULL THEN 'missing_date'
                       WHEN c.RESOLVE_DATE IS NOT NULL
                            AND CAST(c.RESOLVE_DATE AS date) < CAST(COALESCE(c.ONSET_DATE, c.REPORT_DATE) AS date)
            ...
            cond_counts = dict(
                connection.execute(
                    text(cond_cte + " SELECT record_status, COUNT_BIG(*) FROM classified GROUP BY record_status")
                ).fetchall()
            )
            ...
            condition_report_date_fallback = _scalar(
                connection,
                cond_cte
                + " SELECT COUNT_BIG(*) FROM classified WHERE record_status='eligible' AND ONSET_DATE IS NULL AND REPORT_DATE IS NOT NULL",
            )
            ...
            duplicate_diag_ids = _scalar(
                ...
                SELECT COUNT_BIG(*) FROM (
                  SELECT DIAGNOSISID FROM [{source_schema}].[PCORnet_DIAGNOSIS]
                  WHERE DIAGNOSISID IS NOT NULL AND LTRIM(RTRIM(CAST(DIAGNOSISID AS nvarchar(max)))) <> ''
                  GROUP BY DIAGNOSISID HAVING COUNT_BIG(*) > 1
                ) x
                """,
            )
            duplicate_condition_ids = _scalar(
                ...
                f"""
                SELECT COUNT_BIG(*) FROM (
                  SELECT CONDITIONID FROM [{source_schema}].[PCORnet_CONDITION]
                  WHERE CONDITIONID IS NOT NULL AND LTRIM(RTRIM(CAST(CONDITIONID AS nvarchar(max)))) <> ''
                  GROUP BY CONDITIONID HAVING COUNT_BIG(*) > 1
                ) x
                """,
            )
            if duplicate_diag_ids or duplicate_condition_ids:
                raise RuntimeError(
                    ...
                    f"DIAGNOSISID duplicates={duplicate_diag_ids:,}, CONDITIONID duplicates={duplicate_condition_ids:,}"
                )
            ...
            existing = _scalar(
                connection, f"SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence]"
            )
            if existing:
                if existing != expected_rows:
                    raise RuntimeError(
                        f"Target [{target_schema}].[condition_occurrence] already has {existing:,} rows; "
                        f"validated eligible total is {expected_rows:,}. Refusing to append or overwrite."
                    )
                ...
                source_codes AS (
                  ...
                  FROM diag_eligible WHERE DX IS NOT NULL AND vocabulary_id IS NOT NULL
                  UNION
                  SELECT DISTINCT CAST(CONDITION AS nvarchar(255)), vocabulary_id
                  FROM cond_eligible WHERE CONDITION IS NOT NULL AND vocabulary_id IS NOT NULL
                ),
                code_map AS (
                  SELECT sc.source_code, sc.vocabulary_id,
                         src.concept_id AS source_concept_id,
                         CASE
                           WHEN src.standard_concept='S' AND src.domain_id='Condition' AND src.invalid_reason IS NULL
                             THEN src.concept_id
                           ELSE mapped.concept_id
                         END AS standard_concept_id
                  FROM source_codes sc
                  OUTER APPLY (
                    SELECT TOP (1) c.concept_id, c.standard_concept, c.domain_id, c.invalid_reason
                    FROM [{target_schema}].[concept] c
                    WHERE c.concept_code = sc.source_code
                      AND c.vocabulary_id = sc.vocabulary_id
                    ORDER BY CASE WHEN c.invalid_reason IS NULL THEN 0 ELSE 1 END, c.concept_id
                  ) src
                  OUTER APPLY (
                    SELECT TOP (1) target.concept_id
                ...
                combined AS (
                  ...
                  SELECT
                    ...
                    CASE WHEN c.ONSET_DATE IS NOT NULL THEN 'ONSET_DATE' ELSE 'REPORT_DATE' END
                ...
                CREATE TABLE [{target_schema}].[{xwalk_table}] (
                  ...
                  CONSTRAINT PK_{xwalk_table} PRIMARY KEY (source_domain, source_record_id),
                  CONSTRAINT UQ_{xwalk_table}_condition UNIQUE (condition_occurrence_id)
                );

                WITH diag_ids AS (
                  SELECT CAST(DIAGNOSISID AS nvarchar(255)) AS source_record_id,
                         CAST(DX_TYPE AS nvarchar(50)) AS source_code_type,
                         CAST(DX_ORIGIN AS nvarchar(50)) AS source_provenance,
                         'DX_DATE' AS date_basis
                  FROM [{source_schema}].[PCORnet_DIAGNOSIS] d
                  JOIN [{target_schema}].[person] p ON CAST(d.PATID AS nvarchar(50)) = p.person_source_value
                  WHERE d.DIAGNOSISID IS NOT NULL
                    AND LTRIM(RTRIM(CAST(d.DIAGNOSISID AS nvarchar(max)))) <> ''
                    AND d.DX_DATE IS NOT NULL
                ),
                cond_ids AS (
                  SELECT CAST(CONDITIONID AS nvarchar(255)) AS source_record_id,
                         CAST(CONDITION_TYPE AS nvarchar(50)) AS source_code_type,
                         CAST(CONDITION_SOURCE AS nvarchar(50)) AS source_provenance,
                         CASE WHEN ONSET_DATE IS NOT NULL THEN 'ONSET_DATE' ELSE 'REPORT_DATE' END AS date_basis
                  FROM [{source_schema}].[PCORnet_CONDITION] c
                  JOIN [{target_schema}].[person] p ON CAST(c.PATID AS nvarchar(50)) = p.person_source_value
                  WHERE c.CONDITIONID IS NOT NULL
                    AND LTRIM(RTRIM(CAST(c.CONDITIONID AS nvarchar(max)))) <> ''
                    AND COALESCE(c.ONSET_DATE, c.REPORT_DATE) IS NOT NULL
                    AND (c.RESOLVE_DATE IS NULL OR CAST(c.RESOLVE_DATE AS date) >= CAST(COALESCE(c.ONSET_DATE, c.REPORT_DATE) AS date))
                ...
                SELECT source_domain, source_record_id, condition_occurrence_id,
                       source_code_type, source_provenance, date_basis
                FROM numbered_ids;
                """
            ...
            if lineage_rows != target_rows:
                raise RuntimeError(
                    f"Condition lineage reconciliation failed: lineage={lineage_rows:,}, target={target_rows:,}"
                )
            ...
            condition_concept_zero = _scalar(
                connection,
                f"""
                SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence] co
                JOIN [{target_schema}].[{xwalk_table}] x ON x.condition_occurrence_id=co.condition_occurrence_id
            ...
            diagnosis_source_concept_zero = _scalar(
                connection,
                f"""
                SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence] co
                JOIN [{target_schema}].[{xwalk_table}] x ON x.condition_occurrence_id=co.condition_occurrence_id
            ...
            condition_source_concept_zero = _scalar(
                connection,
                f"""
                SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence] co
                JOIN [{target_schema}].[{xwalk_table}] x ON x.condition_occurrence_id=co.condition_occurrence_id
            ...
            diagnosis_visit_linked = _scalar(
                connection,
                f"""
                SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence] co
                JOIN [{target_schema}].[{xwalk_table}] x ON x.condition_occurrence_id=co.condition_occurrence_id
            ...
            condition_visit_linked = _scalar(
                connection,
                f"""
                SELECT COUNT_BIG(*) FROM [{target_schema}].[condition_occurrence] co
                JOIN [{target_schema}].[{xwalk_table}] x ON x.condition_occurrence_id=co.condition_occurrence_id
        ...
        json.dumps(
            {
                ...
                "mapping_strategy": {
                    "diagnosis_date": "DX_DATE required; missing DX_DATE excluded without sentinel",
                    "condition_date": "ONSET_DATE when available; otherwise REPORT_DATE as an explicit source-domain fallback; both absent excluded",
                    "condition_end": "RESOLVE_DATE when present; rows resolving before effective start are excluded",
                    "source_vocabulary": "DX_TYPE/CONDITION_TYPE routed explicitly to ICD9CM, ICD10CM, or SNOMED; unsupported types remain unmapped",
                    "standard_mapping": "source concept followed through current 'Maps to' relationship to an active standard Condition-domain concept; otherwise concept_id 0",
                    "source_lineage": "DIAGNOSIS and CONDITION retained separately; no silent deduplication",
                    "visit_linkage": "ENCOUNTERID linked only when the encounter survived validated visit_occurrence ETL; otherwise visit_occurrence_id is NULL",
                    "provider": "NULL because PCORnet_PROVIDER source is unavailable",
                    "condition_provenance": "CONDITION_TYPE/CONDITION_SOURCE retained in lineage table; OMOP type/status concepts left 0 pending semantic validation",
