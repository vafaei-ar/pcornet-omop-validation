...
def _materialize_temp_semantics(con, source_schema: str, target_schema: str) -> None:
    """Materialize the locked Stage B semantic comparison into connection-local temp tables.

    This preserves the original Stage B definition but prevents SQL Server from re-expanding
    the same multi-million-row CTE for every metric. No persistent source/target table is
    modified; all intermediate objects live only in tempdb for this connection.
    """
    con.exec_driver_sql("SET NOCOUNT ON")

    _progress("materializing source semantic routes")
    con.exec_driver_sql(f"""
        SELECT
            p.person_id,
            CAST(d.DX_DATE AS date) AS event_date,
            CAST(r.target_domain AS varchar(32)) AS target_domain,
            CAST(r.target_concept_id AS bigint) AS target_concept_id,
            CAST(r.source_domain AS varchar(16)) AS source_domain,
            CAST(r.source_record_id AS nvarchar(255)) AS source_record_id,
            CAST(r.route_status AS varchar(64)) AS route_status,
            CAST(r.is_fallback AS bit) AS is_fallback
        INTO #source_semantic
        ...
        SELECT
            ...
            CAST(r.target_domain AS varchar(32)),
            CAST(r.target_concept_id AS bigint),
            CAST(r.source_domain AS varchar(16)),
            CAST(r.source_record_id AS nvarchar(255)),
            CAST(r.route_status AS varchar(64)),
            CAST(r.is_fallback AS bit)
        FROM [{target_schema}].[{ROUTE_TABLE}] r
        ...
        CREATE CLUSTERED INDEX IX_source_semantic_signature
          ON #source_semantic(person_id, event_date, target_domain, target_concept_id);
        CREATE INDEX IX_source_semantic_source
          ON #source_semantic(source_domain, source_record_id);
    """)

    _progress("materializing mapped source routes and concept space")
    con.exec_driver_sql("""
        SELECT * INTO #source_mapped
        FROM #source_semantic
        WHERE target_concept_id <> 0;

        CREATE CLUSTERED INDEX IX_source_mapped_signature
          ON #source_mapped(person_id, event_date, target_domain, target_concept_id);

        SELECT DISTINCT target_domain, target_concept_id
        INTO #source_concepts
        FROM #source_mapped;
    ...
    _progress("scanning native OMOP event tables once into source concept space")
    con.exec_driver_sql(f"""
        CREATE TABLE #target_semantic (
        ...
        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.condition_start_date AS date), 'Condition', CAST(o.condition_concept_id AS bigint)
        FROM [{target_schema}].[condition_occurrence] o
        JOIN #source_concepts s
          ON s.target_domain = 'Condition' AND s.target_concept_id = o.condition_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.observation_date AS date), 'Observation', CAST(o.observation_concept_id AS bigint)
        FROM [{target_schema}].[observation] o
        JOIN #source_concepts s
          ON s.target_domain = 'Observation' AND s.target_concept_id = o.observation_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.procedure_date AS date), 'Procedure', CAST(o.procedure_concept_id AS bigint)
        FROM [{target_schema}].[procedure_occurrence] o
        JOIN #source_concepts s
          ON s.target_domain = 'Procedure' AND s.target_concept_id = o.procedure_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.measurement_date AS date), 'Measurement', CAST(o.measurement_concept_id AS bigint)
        FROM [{target_schema}].[measurement] o
        JOIN #source_concepts s
          ON s.target_domain = 'Measurement' AND s.target_concept_id = o.measurement_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.drug_exposure_start_date AS date), 'Drug', CAST(o.drug_concept_id AS bigint)
        FROM [{target_schema}].[drug_exposure] o
        JOIN #source_concepts s
          ON s.target_domain = 'Drug' AND s.target_concept_id = o.drug_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.device_exposure_start_date AS date), 'Device', CAST(o.device_concept_id AS bigint)
        FROM [{target_schema}].[device_exposure] o
        JOIN #source_concepts s
          ON s.target_domain = 'Device' AND s.target_concept_id = o.device_concept_id;

        INSERT INTO #target_semantic
        SELECT o.person_id, CAST(o.specimen_date AS date), 'Specimen', CAST(o.specimen_concept_id AS bigint)
        FROM [{target_schema}].[specimen] o
        JOIN #source_concepts s
          ON s.target_domain = 'Specimen' AND s.target_concept_id = o.specimen_concept_id;

        CREATE CLUSTERED INDEX IX_target_semantic_signature
          ON #target_semantic(person_id, event_date, target_domain, target_concept_id);
    """)

    _progress("aggregating semantic signatures")
    con.exec_driver_sql("""
        SELECT person_id, event_date, target_domain, target_concept_id, COUNT_BIG(*) AS n
        INTO #source_signature
        FROM #source_mapped
        GROUP BY person_id, event_date, target_domain, target_concept_id;
        CREATE UNIQUE CLUSTERED INDEX IX_source_signature
          ON #source_signature(person_id, event_date, target_domain, target_concept_id);

        SELECT person_id, event_date, target_domain, target_concept_id, COUNT_BIG(*) AS n
        INTO #target_signature
        FROM #target_semantic
        GROUP BY person_id, event_date, target_domain, target_concept_id;
        CREATE UNIQUE CLUSTERED INDEX IX_target_signature
          ON #target_signature(person_id, event_date, target_domain, target_concept_id);
    """)
...
def run(config_path: str, output_dir: str | None = None) -> dict[str, object]:
    ...
    out = (
        Path(output_dir)
        if output_dir
        else config.audit_dir.parent / "publication_analysis" / "stage_b_patient_concordance" / "condition"
    )
    ...
    try:
        with engine.connect() as con:
            ...
            _materialize_temp_semantics(con, source_schema, target_schema)

            _progress("computing aggregate concordance metrics")
            source_core_route_rows = _scalar(con, "SELECT COUNT_BIG(*) FROM #source_semantic")
            source_mapped_route_rows = _scalar(con, "SELECT COUNT_BIG(*) FROM #source_mapped")
            unresolved_fallback_rows = _scalar(con, "SELECT COUNT_BIG(*) FROM #source_semantic WHERE target_concept_id = 0")
            unresolved_fallback_patients = _scalar(con, "SELECT COUNT_BIG(DISTINCT person_id) FROM #source_semantic WHERE target_concept_id = 0")
            target_semantic_rows = _scalar(con, "SELECT COUNT_BIG(*) FROM #target_semantic")

            source_event_row = con.execute(text("""
                SELECT COUNT_BIG(*), SUM(CASE WHEN n > 1 THEN 1 ELSE 0 END)
                FROM (
            ...
            patient_row = con.execute(text("""
                WITH s AS (SELECT DISTINCT person_id FROM #source_mapped),
            ...
            signature_row = con.execute(text("""
                SELECT
                ...
                FROM #source_signature s
                FULL OUTER JOIN #target_signature t
                  ON t.person_id=s.person_id
                 AND t.event_date=s.event_date
                 AND t.target_domain=s.target_domain
            ...
            for domain in EVENT_DOMAINS:
                row = con.execute(text("""
                    SELECT
                        ...
                        COUNT_BIG(DISTINCT CASE WHEN s.n IS NOT NULL THEN s.person_id END),
                        COUNT_BIG(DISTINCT CASE WHEN t.n IS NOT NULL THEN t.person_id END),
                    ...
                    FROM (SELECT * FROM #source_signature WHERE target_domain=:domain) s
                    FULL OUTER JOIN (SELECT * FROM #target_signature WHERE target_domain=:domain) t
                      ON t.person_id=s.person_id
                     AND t.event_date=s.event_date
                     AND t.target_concept_id=s.target_concept_id
                """), {"domain": domain}).one()
    ...
    summary = {
        ...
        "implementation": "optimized_temp_table_materialization_v1",
        "semantic_reference": (
        ...
        "interpretation_rules": [
            ...
            "Temporary tables are connection-local performance intermediates only; persistent source and target data are read-only.",
        ],
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run optimized Stage B Wave 1 Condition semantic concordance")
